...
public class DriverDao {
	...
	public List<DriverVo.Response> getDataSuperVisor(String status) {
	   Employee employee = 	employeeRepo.findByUsername(UserLoginUtil.getUsername());
		String sql = "SELECT w.worksheet_id, w.worksheet_date, w.worksheet_time_begin, bl.bus_lines_no, "
				...
				+ "WHERE w.bus_lines_id = ? AND w.worksheet_status = ? ";
	...
	public List<DriverVo.Response> getDataDriver(String status) {
		String sql = "SELECT w.worksheet_id, w.worksheet_date, w.worksheet_time_begin, bl.bus_lines_no, "
				+ "w.worksheet_time_end, w.worksheet_status, w.bus_vehicle_plate_no, "
				+ "w.create_by, w.create_date, w.update_by, w.update_date, w.worksheet_hours, "
				+ "w.worksheet_hours_ot, w.bus_lines_id, w.bus_division_id, w.bus_vehicle_id, "
				...
				+ "LEFT JOIN employee terminal_agent ON w.worksheet_terminal_agent = terminal_agent.username "
				+ "LEFT JOIN employee buslines_manager ON w.worksheet_buslines_manager = buslines_manager.username "
				+ "WHERE w.worksheet_driver = ? AND w.worksheet_status = ? ";
	...
	public List<DriverVo.Response> getDataDriverOrEnd(String status,String statusEnd) {
		String sql = "SELECT w.worksheet_id, w.worksheet_date, w.worksheet_time_begin, bl.bus_lines_no, "
				+ "w.worksheet_time_end, w.worksheet_status, w.bus_vehicle_plate_no, "
				+ "w.create_by, w.create_date, w.update_by, w.update_date, w.worksheet_hours, "
				+ "w.worksheet_hours_ot, w.bus_lines_id, w.bus_division_id, w.bus_vehicle_id, "
				...
				+ "LEFT JOIN employee terminal_agent ON w.worksheet_terminal_agent = terminal_agent.username "
				+ "LEFT JOIN employee buslines_manager ON w.worksheet_buslines_manager = buslines_manager.username "
				+ "WHERE w.worksheet_driver = ? AND w.worksheet_status = ? or  w.worksheet_status  = ? ";

		List<Object> params = new ArrayList<>();
		params.add(UserLoginUtil.getUsername());
		params.add(status);
		params.add(statusEnd);
		System.out.println(sql);
		return commonJdbcTemplate.executeQuery(sql, params.toArray(),
				BeanPropertyRowMapper.newInstance(DriverVo.Response.class));
	}
	...
	public List<DriverVo.Response> getDataFarecollectOrEnd(String status,String statusEnd) {
		String sql = "SELECT w.worksheet_id, w.worksheet_date, w.worksheet_time_begin, bl.bus_lines_no, "
				+ "w.worksheet_time_end, w.worksheet_status, w.bus_vehicle_plate_no, "
				+ "w.create_by, w.create_date, w.update_by, w.update_date, w.worksheet_hours, "
				+ "w.worksheet_hours_ot, w.bus_lines_id, w.bus_division_id, w.bus_vehicle_id, "
				...
				+ "LEFT JOIN employee terminal_agent ON w.worksheet_terminal_agent = terminal_agent.username "
				+ "LEFT JOIN employee buslines_manager ON w.worksheet_buslines_manager = buslines_manager.username "
				+ "WHERE w.worksheet_farecollect = ? AND w.worksheet_status = ? or  w.worksheet_status  = ? ";
